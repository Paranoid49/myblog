...
def list_admin_posts_api(request: Request, db: Session = Depends(get_db)) -> JSONResponse:
    init_error = _require_initialized(db)
    if init_error:
        return init_error
    ...
    posts = list(db.execute(select(Post).order_by(Post.created_at.desc())).scalars().all())
    return _ok([_serialize_post(post) for post in posts])
